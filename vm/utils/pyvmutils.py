...
def get_all_hosts_info(content):
    ...
    for host in containerView.view:
        hosts[host.name] = {
            ...
            'cpuModel': host.summary.hardware.cpuModel,  # CPU型号
            'cpuMhz': host.summary.hardware.cpuMhz,  # CPU频率
            'cpuusage': '%.1f%%' % (host.summary.quickStats.overallCpuUsage / (
                        host.summary.hardware.numCpuPkgs * host.summary.hardware.numCpuCores * host.summary.hardware.cpuMhz) * 100),
        # 处理器使用率
            'numCpuPkgs': host.summary.hardware.numCpuPkgs,  # 物理插槽
            'numCpuCores': host.summary.hardware.numCpuCores,  # CPU核心
            'numCpuThreads': host.summary.hardware.numCpuThreads,  # 逻辑CPU个数
            'memorysize': '%.1f GB' % (((
                                                    host.summary.hardware.memorySize / 1024 / 1024) - host.summary.quickStats.overallMemoryUsage) / 1024),
        # 可用内存(GB)
            ...
            'overallmemory': '%.1f%%' % ((host.summary.quickStats.overallMemoryUsage / (
...
def clone_vm(content, template, vm_name, datacenter_name, vm_folder, datastore_name, resource_pool, power_on, numcpu,
    ...
    clonespec = vim.vm.CloneSpec(powerOn=power_on, template=False, location=relospec, customization=customspec,
                                 config=specconfig)
